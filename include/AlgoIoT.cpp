...
// was successfully submitted to the Algorand blockchain
  ...
  printf("\t Transaction successfully submitted with ID=");
...
int AlgoIoT::getAlgorandTxParams(uint32_t *round, uint16_t *minFee)
{
  ...
  *minFee = 0;
  ...
  int httpResponseCode = -1; // m_httpClient.GET(); // Temporarily disabled

  // httpResponseCode will be negative on error
  if (httpResponseCode < 0)
  {
#ifdef LIB_DEBUGMODE
    // DEBUG_SERIAL.print("HTTP GET failed, error: ");
    // DEBUG_SERIAL.println(m_httpClient.errorToString(httpResponseCode).c_str());
    printf("HTTP GET failed, error: ");
#endif
    return ALGOIOT_INTERNAL_GENERIC_ERROR;
  }
  else
  {
    ...
    { // No error: let's get the response
      ...
      printf("GetParams server response:");
#endif
      ...
      if (false) //(error)
      {
        ...
        printf("GetParams: JSON response parsing failed!");
#endif
        return ALGOIOT_INTERNAL_GENERIC_ERROR;
      }
      else
      { // Fetch interesting fields
        ...
        *round = JSONResDoc["last-round"];
        ...
        // DEBUG_SERIAL.print("min-fee = ");
        // DEBUG_SERIAL.print(*minFee);
        // DEBUG_SERIAL.println(" microAlgo");
        // DEBUG_SERIAL.print("last-round = ");
        // DEBUG_SERIAL.println(*round);
        printf("GetParams: JSON response parsing failed!");
#endif
      }
    }
    ...
    { // No error, but no data available from server
      ...
      printf("Server returned no data");
#endif
      return ALGOIOT_NETWORK_ERROR;
    }
    break;
    ...
    {
#ifdef LIB_DEBUGMODE
      // DEBUG_SERIAL.print("Unmanaged HTTP response code ");
      // DEBUG_SERIAL.println(httpResponseCode);
      printf("Unmanaged HTTP response code ");
#endif
      return ALGOIOT_INTERNAL_GENERIC_ERROR;
    }
    break;
...
int AlgoIoT::submitTransaction(msgPack msgPackTx)
{
  ...
  else
  {
    ...
    { // Malformed request
      ...
      // String payload = "0"; // m_httpClient.getString();
      // DEBUG_SERIAL.println(payload);
      printf("\nTransaction format error");
#endif
      return ALGOIOT_TRANSACTION_ERROR;
